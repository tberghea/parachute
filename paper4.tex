...
\author{C. T. Berghea\altaffilmark{1}, G. Nelson\altaffilmark{1}, C. E. Rusu\altaffilmark{2}, C. R. Keeton\altaffilmark{3}}
...
\tableline
\multicolumn{3}{l}{\tiny {Model prediction. Isothermal ellipsoid}}\\
\tableline
...
\tableline
\multicolumn{3}{l}{\tiny {Model prediction. Isothermal sphere with external shear}}\\
\tableline
...
The first five rows are magnitude measurements derived using Galfit and recalibrated using PS1 photometry for nearby stars and corrected using NED extinction calculator.
The following three rows are relative i magnitudes and relative positions in arcseconds obtained from Galfit. The reference magnitude is the i magnitude of source A and reference position is the average between the i and y bands for source A.
The next row shows the photometric redshift estimates following \citet{wu10}.
Following rows shows the model prediction for position and relative magnitudes for the two gravitational models we tried.
}
\end{deluxetable}
...
\begin{figure}
\epsscale{1.1}
\plottwo{redshift5.eps}{redshift4.eps}
\caption{
